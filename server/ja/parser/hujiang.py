...
class Hujiang(object):
    ...
    def search(self, word):
        ...
        results = []
        for item in doc("div.word-details-pane"):
            result = {'word': word}
            item = PyQuery(item)
            result['kanji'] = item("div.word-text h2").eq(0).text()
            result['kana'] = item("div.pronounces span").eq(0).text().replace('[', '').replace(']', '')
            simple_definition = item("span.simple-definition").text()
            if simple_definition:
                result['meaning'] = simple_definition
            else:
                result['meaning'] = item("section.detail-groups").html()
