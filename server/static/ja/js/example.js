...
var app = new Vue({
  ...
  mounted () {
    this.compare_dt = new Date("2017-08-25 00:00:00").getTime()
  ...
  methods: {
    loadTextArea: function (){
      words = this.text_of_textarea.split('\n');
      word = words.shift()
      this.data.word = word
      this.text_of_textarea = words.join('\n');
      ...
      if(test){
        ...
        .done(function (res, ts, j) {
          if(res.status=='duplicated'){
            if(new Date(res.result.updated).getTime() > root.compare_dt){
              console.log(new Date(res.result.updated).getTime(), root.compare_dt)
              root.reset();
              root.loadTextArea()
              return
            }
  ...
  if (event.ctrlKey || event.metaKey) {
    switch (String.fromCharCode(event.which).toLowerCase()) {
      ...
      case 'd':
          event.preventDefault();
          app.update_edit()
          app.search_example(false, app.data.word)
          break;
      case 'e':
          event.preventDefault();
          app.reset();
