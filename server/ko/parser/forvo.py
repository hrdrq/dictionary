...
class Forvo(object):
    URL = 'https://ja.forvo.com/search/{word}/ko/'
    ...
    def parse_items(self, urls):
        ...
        for item_response_body in docs:
            ...
            for locale in item_doc("article.pronunciations"):
                ...
                ja_header = locale('em[id=ko]')
                if ja_header:
                    word = re.compile(
                        r"(.*) の発音").search(ja_header.text()).group(1)
                    for i in locale('a.play'):
                        i = PyQuery(i)
                        ...
                        if match:
                            ...
                            self.results.append({'word': word, 'url': url})
    ...
    def add(self, word):
        url = "https://ja.forvo.com/word-add/{word}?word={word}&id_lang=86".format(
            word=word)
        s = requests.Session()
        s.post('https://ja.forvo.com/login/',
               data={'login': FORVO_USER, 'password': FORVO_PW})
        res = s.get(url)
